...
```r
world <- get_stamenmap(bbox = c(left = -180, bottom = -57, right = 179, top = 82),
                       maptype = "terrain",
                       zoom = 2)
```
...
**This map can give us hints about the laws of various countries. I can recognize chunks of blue, purple, and orange in Asia; whereas the US is exclusively pink and purple etc.**

  2. Construct a new map of Starbucks locations in the Twin Cities metro area (approximately the 5 county metro area).  
  

```r
twinCities <- get_stamenmap(bbox = c(left = -93.7, bottom = 44.7, right = -92.7, top = 45.2),
                       maptype = "terrain",
                       zoom = 10)
```
...
```
## Warning: Removed 25458 rows containing missing values (geom_point).
```
...
**The zoom number is the level of detail on the map. Higher zoom : more detail**
...
ggmap(twinCities) +
  geom_point(data = Starbucks,
             aes(x = Longitude, y = Latitude)) +
  annotate("text", x = 45, y = -93, label = "Macalester College") + #y u no show up :(
  theme_map()
```
...
**160 reads in a csv to create a dataset. 161-163 clean up the state variable so it matches the Starbucks state variable. 165-168 then creates a new dataset by joining the two by their respective state variable names. Finally, 169 adds a new variable that calculates the number of locations per 10,000 citizens.**
